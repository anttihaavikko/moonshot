...
namespace Anima2D
{
    public static class MathUtils
    {
        ...
        public static float SegmentDistance(Vector3 point, Vector3 a, Vector3 b)
        {
            ...
            if (dot <= 0f) return p.magnitude;

            if (dot >= v.sqrMagnitude) return (point - b).magnitude;
        ...
        public static float SegmentSqrtDistance(Vector3 point, Vector3 a, Vector3 b)
        {
            ...
            if (dot <= 0f) return p.sqrMagnitude;

            if (dot >= v.sqrMagnitude) return (point - b).sqrMagnitude;
        ...
        public static Rect ClampRect(Rect rect, Rect frame)
        {
            return new Rect
            {
                xMin = Mathf.Clamp(rect.xMin, 0f, frame.width - 1),
                yMin = Mathf.Clamp(rect.yMin, 0f, frame.height - 1),
        ...
        {
            if (minDifference == 0f) return DiscardLeastSignificantDecimal(valueToRound);
