...
namespace Anima2D
{
    public class PreviewGameObjectCreationPolicy : InstantiateCreationPolicy<GameObject>
    {
        ...
        public override GameObject Create()
        {
            ...
            if (l_instance) EditorExtra.InitInstantiatedPreviewRecursive(l_instance);
    ...
    {
        private OnionLayerGameObjectPool m_GameObjectPool;
        private readonly OnionLayerPool m_OnionLayerPool = new OnionLayerPool();
        private readonly Dictionary<int, OnionLayer> m_OnionLayers = new Dictionary<int, OnionLayer>();
        ...
        public GameObject source
        {
            ...
            set
            {
                if (m_Source != value)
                {
                    ...
                    if (m_Source) m_GameObjectPool = new OnionLayerGameObjectPool(m_Source);
                }
            }
        }

        private OnionLayer GetOnionLayer(int frame, AnimationClip clip)
        {
            ...
            if (!m_OnionLayers.TryGetValue(frame, out l_onionLayer))
            {
                ...
                if (!l_onionLayer.previewInstance) l_onionLayer.SetPreviewInstance(m_GameObjectPool.Get(), source);
        ...
        private void ReturnOnionLayers(int minFrame, int maxFrame, int step)
        {
            ...
            Color colorPrevFrames, Color colorNextFrames)
        {
            ...
            var frameCount = (int) (AnimationWindowExtra.activeAnimationClip.length *
