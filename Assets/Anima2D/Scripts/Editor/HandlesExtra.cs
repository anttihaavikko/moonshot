...
namespace Anima2D
{
    public class HandlesExtra
    {
        ...
        private static MethodInfo s_ApplyWireMaterialMethodInfo;

        private static Vector2 s_CurrentMousePosition;
        private static Vector2 s_DragStartScreenPosition;
        private static Vector2 s_DragScreenOffset;

        private static Vector3[] s_circleArray;

        private static Vector3[] s_array;
            ...
            Vector3 planePosition)
        {
            ...
            switch (eventType)
            {
                ...
                case EventType.MouseDrag:
                    if (GUIUtility.hotControl == id)
                    {
                        ...
                        if (!Mathf.Approximately((center - position).magnitude, 0f)) GUI.changed = true;
                        Event.current.Use();
                    }
            ...
            if (drawCapFunction != null) drawCapFunction(id, position, Quaternion.identity, 1f, eventType);
        ...
        private static void DrawImageBasedCap(int controlID, Vector3 position, Quaternion rotation, float size,
        ...
        public static void ApplyWireMaterial()
        {
            ...
            if (s_ApplyWireMaterialMethodInfo != null) s_ApplyWireMaterialMethodInfo.Invoke(null, null);
        }

        private static void SetDiscSectionPoints(Vector3[] dest, int count, Vector3 normal, Vector3 from, float angle)
        {
            from.Normalize();
            var rotation = Quaternion.AngleAxis(angle / (count - 1), normal);
            var vector = from;
        ...
        {
            if (Event.current.type != EventType.Repaint || Handles.color.a == 0f) return;
            ...
            Color color)
        {
            ...
            if (s_array == null) s_array = new Vector3[60];
        ...
        public static void DrawAAPolyLine(Color[] colors, Vector3[] points)
        {
            ...
            var method = type.GetMethod("DrawAAPolyLine",
                BindingFlags.Static | BindingFlags.Public | BindingFlags.NonPublic, null,
                new[] {typeof(Color[]), typeof(Vector3[])}, null);

            if (method != null)
            {
                object[] parameters = {colors, points};
