...
namespace Anima2D
{
    public class SpriteMeshCache : SerializedCache, IVertexManipulable, IRectManipulable
    {
        ...
        [SerializeField] private SpriteMeshEditorWindow.Mode m_Mode;

        public List<Node> nodes = new List<Node>();

        [SerializeField] private List<Vector2> m_TexVertices = new List<Vector2>();
        ...
        [SerializeField] private VertexSelection m_Selection = new VertexSelection();

        public Edge selectedEdge;

        [SerializeField] private int mSelectedHoleIndex = -1;

        public Bone2D selectedBone;

        [SerializeField] private int m_SelectedBindPose = -1;

        [SerializeField] private BlendShape m_SelectedBlendShape;

        [SerializeField] private RectManipulatorParams m_RectManipulatorParams;

        [SerializeField] private bool m_DirtyVertices = true;

        [SerializeField] private List<Vector2> m_CurrentTexVertices = new List<Vector2>();

        //[SerializeField]
        private float m_BlendShapeWeight;
        ...
        private List<IndexedEdge> indexedEdges
        {
            get { return edges.ConvertAll(e => new IndexedEdge(nodes.IndexOf(e.node1), nodes.IndexOf(e.node2))); }
        }

        public VertexSelection selection => m_Selection;

        public Node selectedNode
        {
            get
            {
                if (selection.Count == 1) return nodes[selection.First()];
        ...
        public bool multiselection => selection.Count > 1;

        public bool isBound => bindPoses.Count > 0f;
        ...
        public Hole selectedHole
        {
            get
            {
                ...
                if (mSelectedHoleIndex >= 0 && mSelectedHoleIndex < holes.Count) hole = holes[mSelectedHoleIndex];
        ...
        {
            get => GetBlendShapeFrame();
            set
            {
                if (selectedBlendshape && selectedBlendshape.frames.Contains(value)) blendShapeWeight = value.weight;
            }
        }

        public float blendShapeWeight
        {
            get => m_BlendShapeWeight;
            set
            {
        ...
        public override void RegisterUndo(string undoName)
        {
        ...
        {
            for (var i = 0; i < nodes.Count; ++i) nodes[i].index = i;
        }
        ...
        public void ApplyChanges()
        {
            ...
            if (spriteMeshInstance) SpriteMeshUtils.UpdateRenderer(spriteMeshInstance, false);
        }
        ...
        public void RevertChanges()
        {
            ...
            if (spriteMesh && spriteMeshData)
            {
                ...
                if (boneWeights.Count != nodes.Count) boneWeights = nodes.ConvertAll(n => BoneWeight.Create());
        ...
        public void Clear(string undoName)
        {
            ...
            foreach (var edge in edges) DestroyObjectImmediate(edge);

            foreach (var node in nodes) DestroyObjectImmediate(node);
        ...
        public void DeleteNode(Node node, bool triangulate = true)
        {
            ...
            {
                var edge = edges[i];
                if (edge.ContainsNode(node)) l_edges.Add(edge);
            }
            ...
            {
                foreach (var edge in l_edges) edges.Remove(edge);
            }
        ...
        public void DeleteBlendShape(BlendShape blendshape, string undoName = "")
        {
            if (blendshape)
            {
                ...
                foreach (var frame in blendshape.frames) DestroyObjectImmediate(frame);
        ...
        public void ResetVertices(List<Node> _nodes, string undoName = "")
        {
            if (selectedBlendshapeFrame)
            {
                ...
                foreach (var node in _nodes) SetVertex(node, m_TexVertices[node.index]);
        ...
        private void CreateBlendShapeCache(SpriteMeshData spriteMeshData)
        {
            ...
            foreach (var blendShape in spriteMeshData.blendshapes)
            {
                ...
                foreach (var frame in blendShape.frames)
                {
                    var frameClone = CreateInstance<BlendShapeFrame>();
        ...
        private void DestroyBlendShapeCache(string undoName)
        {
            foreach (var blendShape in blendshapes)
                if (blendShape)
                {
        ...
        private void SetBlendShapesFromCache()
        {
            if (spriteMesh)
                foreach (var blendshape in blendshapes)
                {
        ...
        private void SortBlendshapeFrames(BlendShape blendShape)
        {
            if (blendShape)
            {
        ...
        public void SetBlendShapeFrameWeight(BlendShapeFrame blendShapeFrame, float weight, string undoName)
        {
                ...
                selectedBlendshape.frames.Contains(blendShapeFrame))
            {
                ...
                if (other) DeleteBlendShapeFrame(selectedBlendshape, other, undoName);
            ...
            string undoName)
        {
            ...
            var clampedRect = MathUtils.ClampRect(MathUtils.OrderMinMax(l_rect),
        ...
        private bool ContainsVector(Vector2 vectorToFind, List<Vector2> list, float epsilon, out int index)
        {
        ...
        public void DeleteBindPose(BindInfo bindPose)
        {
            if (bindPose)
            {
                if (selectedBindPose == bindPose) selectedBindPose = null;
        ...
        public void BindBones()
        {
            ...
            if (spriteMeshInstance)
            {
                ...
                foreach (var bone in spriteMeshInstance.bones) BindBone(bone);
            }
        }
        ...
        public void CalculateAutomaticWeights(List<Node> targetNodes)
        {
            ...
            var boneWeights = BbwPlugin.CalculateBbw(m_TexVertices.ToArray(), indexedEdges.ToArray(),
                controlPoints.ToArray(), controlPointEdges.ToArray(), pins.ToArray());
        ...
        private BoneWeight CreateBoneWeightFromUnityBoneWeight(UnityEngine.BoneWeight unityBoneWeight)
        {
        ...
        private void FillBoneWeights(List<Node> targetNodes, float[,] weights)
        {
            ...
            foreach (var node in targetNodes)
            {
                ...
                for (var i = 0; i < bindPoses.Count; ++i) l_weights.Add(weights[i, node.index]);

                SetBoneWeight(node, CreateBoneWeightFromWeights(l_weights));
            }
        }

        private BoneWeight CreateBoneWeightFromWeights(List<float> weights)
        {
            var boneWeight = new BoneWeight();
        ...
        private List<Vector3> ToVector3List(List<Vector2> list)
        {
            return list.ConvertAll(v => (Vector3) v);
        }

        private List<Vector3> ToVertices(List<Vector2> list)
        {
            var pixelsPerUnit = SpriteMeshUtils.GetSpritePixelsPerUnit(spriteMesh.sprite);
        ...
        private Vector3 ToVertex(Vector2 v)
        {
            var pixelsPerUnit = SpriteMeshUtils.GetSpritePixelsPerUnit(spriteMesh.sprite);
        ...
        private Vector3 ToVertex(Vector2 v, float pixelsPerUnit)
        {
        ...
        private List<Vector2> GetBlendshapePositions(BlendShape blendshape, float weight)
        {
            ...
            if (blendshape)
            {
                    ...
                    prevFrameVertices.Length == nextFrameVertices.Length)
                {
                    ...
                    if (weightDelta > 0f) t = (weight - prevWeight) / weightDelta;
        ...
        private BlendShapeFrame GetBlendShapeFrame()
        {
            BlendShapeFrame frame = null;
        ...
        public Vector2 GetVertex(Node node)
        {
            ...
            if (node) return m_CurrentTexVertices[node.index];
        ...
        private List<Node> m_ManipulableNodes = new List<Node>();
        ...
        private readonly RectManipulatorData m_RectManipulatorData = new RectManipulatorData();

        public IRectManipulatorData rectManipulatorData => m_RectManipulatorData;
