using UnityEditor;
using UnityEngine;

namespace Anima2D
{
    [InitializeOnLoad]
    public class AnimationWindowExtra
    {
        private static readonly IAnimationWindowImpl s_Impl;

        static AnimationWindowExtra()
        {
#if UNITY_5_0
			s_Impl = new AnimationWindowImpl_50();
#elif UNITY_5_1 || UNITY_5_2 || UNITY_5_3 || UNITY_5_4 || UNITY_5_5
			s_Impl = new AnimationWindowImpl_51_52_53_54_55();
#elif UNITY_5_6
			s_Impl = new AnimationWindowImpl_56();
#elif UNITY_2017_1_OR_NEWER
            s_Impl = new AnimationWindowImpl_2017_1();
#endif
            s_Impl.InitializeReflection();
        }

        public static EditorWindow animationWindow => s_Impl.animationWindow;

        public static int frame
        {
            get => s_Impl.frame;
            set => s_Impl.frame = value;
        }

        public static bool recording
        {
            get => s_Impl.recording;
            set => s_Impl.recording = value;
        }

        public static AnimationClip activeAnimationClip => s_Impl.activeAnimationClip;

        public static GameObject activeGameObject => s_Impl.activeGameObject;

        public static GameObject rootGameObject => s_Impl.rootGameObject;

        public static int refresh => s_Impl.refresh;

        public static float currentTime => s_Impl.currentTime;

        public static bool playing => s_Impl.playing;

        public static float FrameToTime(int frame)
        {
            return s_Impl.FrameToTime(frame);
        }

        public static float TimeToFrame(float time)
        {
            return s_Impl.TimeToFrame(time);
        }
    }
}